...
import huasked.hubosim.PPPaths.PathPoint;
...
public class PurePursuit {
   ...
   public ArrayList<PathPoint> actualPath = new ArrayList<>();

   public PurePursuit(TankDrive chassis, double kPt, double kPd, double trackWidth) {
   ...
   public ArrayList<Map.Entry<Line, Double>> processPath(final ArrayList<PathPoint> unprocessedPath) {
       ArrayList<Map.Entry<Line, Double>> path = new ArrayList<>();
       for (int i = 0; i < unprocessedPath.size() - 1; i++) {
           Line line = new Line(unprocessedPath.get(i).point, unprocessedPath.get(i + 1).point);
           path.add(new HashMap.SimpleEntry<>(line, unprocessedPath.get(i).speed));
       }
       return path;
   }
   ...
   public ArrayList<Line> getStrippedPath(final ArrayList<PathPoint> unprocessedPath) {
   ...
   public void followPath(final ArrayList<PathPoint> unprocessedPath, double lookaheadDistance, double maxSpeed, int segOffset) {
       ...
       actualPath.add(new PathPoint(currentPose.x, currentPose.y, 0));
       indexOfSpeed = (pathSegIndex - segOffset);
       double speedMultiplier = unprocessedPath.get(Math.max(0, indexOfSpeed)).speed;
