...
public class Lidar extends BaseRenderObject{
    ...
    public void debugInfo(Graphics g, int x, int y) {
        String lidarDistances = String.format("FRONT : %.2f | REAR : %.2f | LEFT : %.2f | RIGHT : %.2f", distFromWall[0], distFromWall[2], distFromWall[3], distFromWall[1]);
        g.drawString(lidarDistances, x, y);
    }

    public void drawSensorLines(Graphics2D g) {
    ...
    public void checkForIntersections(Line[] walls) {
        ...
        for (Direction dir : Direction.values()) {
            for (Line wall : walls) {
                ...
                Point intersection = getLineIntersection(sensor, wall);
                if (intersection != null) {
                    ...
                    if (distance < inchesToPixels(maxRange)) {
                        distFromWall[dir.ordinal()] = pixelsToInches(distance) + getGaussianError(1.0);
